...
public class PretService {
    ...
    public void rendrePret(Long pretId) {
        ...
        if (statutRendu == null) throw new IllegalStateException("Statut 'rendu' introuvable");
     ...
     */
    public Pret emprunterLivre(Long adherentId, Long livreId, String typePret) {
        ...
        if (!abonnementService.hasAbonnementActif(adherent, now)) {
            throw new IllegalStateException("Vous n'avez pas d'abonnement actif à la date de l'emprunt. Veuillez renouveler votre abonnement.");
        }
        ...
        pret.setDateEmprunt(java.time.LocalDateTime.now());
        pret.setDateRetourPrevue(java.time.LocalDateTime.now().plusDays(nbJoursPret));
    ...
    public Pret emprunterLivreDirect(Long adherentId, Long livreId, String typePret, Long biblioId) {
        ...
        if (nbJoursPret != null) {
            ...
            pret.setDateRetourPrevue(java.time.LocalDateTime.now());
        }
        String message = null;
        if (eligible) {
            StatutValidation statutValide = statutValidationRepository.findById(2L).orElse(null);
    ...
    /**
     * Calcule la vraie date limite de retour en tenant compte des prolongations acceptées, week-ends, jours fériés et configuration.
     */
    public LocalDate getDateLimiteEffective(Pret pret) {
        ...
        if (isFerie) {
            ...
            // Vérifier à nouveau si on tombe sur un week-end après avoir évité le jour férié
            if (date.getDayOfWeek().getValue() == 6) { // Samedi
                var configWeekend = configurationRepository.findByNom("weekend_direction");
                if (configWeekend != null && configWeekend.getValeur() != null && configWeekend.getValeur() == -1) {
    ...
    /**
     * Retourne les prêts en cours avec la vraie date limite et l'état du bouton pénaliser.
     */
    public List<PretEnCoursDto> getPretsEnCoursDto() {
        ...
        if (statutEnCours == null && statutProlongement == null) return List.of();
        ...
        return pretRepository.findAll().stream()
            .filter(pret -> {
                ...
                return sp != null && (
                    (statutEnCours != null && sp.getId().equals(statutEnCours.getId())) ||
                        (statutProlongement != null && sp.getId().equals(statutProlongement.getId()))
                );
            })
            .map(pret -> {
                LocalDate dateLimite = getDateLimiteEffective(pret);
                boolean isProlongement = pret.getStatutPret() != null && pret.getStatutPret().getLibelle() != null && pret.getStatutPret().getLibelle().equalsIgnoreCase("en prolongement");

                // Vérifier s'il existe déjà une pénalité pour ce prêt spécifique et cet adhérent
                ...
                if (pret.getAdherent() != null) {
                    ...
                    dejaPenalisePourCePret = penalites.stream().anyMatch(p -> p.getPersonne() != null && p.getPersonne().getId().equals(pret.getAdherent().getId()));
                }

                // Le bouton est actif si :
                // - Il y a un retard (now > dateLimite) - dateLimite inclut maintenant les prolongations
    ...
    public void penaliserPret(Long idPret) {
        ...
        int joursPenalite = quotaTrouve != null && quotaTrouve.getNombreJourPenalite() != null ? quotaTrouve.getNombreJourPenalite() : 5;
